...
const bookWeeks: any[] = chessKnowledgeRaw as any[];
...
function buildCurriculum(): WeekMeta[] {
  ...
  // Prefer book weeks from chess_knowledge.json (generated by fetch_chess_knowledge.py)
  // Fall back to hardcoded STANDARD_WEEKS if knowledge file not yet generated
  const bookPool = bookWeeks.length > 0 ? bookWeeks : STANDARD_WEEKS;
  const standardWeeks = bookPool.slice(0, needed).map((w: any, i: number) => ({ key: w.id ?? `standard_${i}`, ...w, count: 0, examples: [] }));
...
function DrillView({ pool, week, onBack, onMarkComplete, isComplete }: {
  pool: any[]; week: any; onBack: () => void; onMarkComplete: () => void; isComplete: boolean;
}) {
  const [index, setIndex] = useState(0);
  const [revealed, setRevealed] = useState(false);
  const [tab, setTab] = useState<"theory" | "drill">("theory");
  ...
  function goTo(n: number) { setIndex(Math.max(0, Math.min(total - 1, n))); setRevealed(false); }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 940 }}>
      ...
      <div style={{ display: "flex", gap: 0, border: "1px solid var(--border)", borderRadius: 7, overflow: "hidden", alignSelf: "flex-start" }}>
        {(["theory", "drill"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: "7px 18px", background: tab === t ? "var(--bg-3)" : "transparent", border: "none", color: tab === t ? "var(--text)" : "var(--text-muted)", fontSize: "0.8rem", fontWeight: tab === t ? 600 : 400, cursor: "pointer" }}>
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 36, alignItems: "flex-start", flexWrap: "wrap" }}>
                <div style={{ flexShrink: 0 }}>
                  <Chessboard options={{ position: blunder.fen_before, boardOrientation: blunder.color === "white" ? "white" : "black", boardStyle: { width: 420, height: 420 }, darkSquareStyle: { backgroundColor: "#b58863" }, lightSquareStyle: { backgroundColor: "#f0d9b5" }, arrows, allowDragging: false }} />
    ...
    return (
      <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", padding: "32px 24px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <DrillView pool={(curriculum[openWeek] as any).examples ?? []} week={curriculum[openWeek]} onBack={() => setOpenWeek(null)} onMarkComplete={() => markComplete(openWeek)} isComplete={completedWeeks.includes(openWeek)} />
        </div>
      </div>
    );
  }
